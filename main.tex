...
Given some simulated data, this paper presents a hierarchical Bayesian framework to infer ozone concentrations as well as some hyper-parameters, such as the noise covariance and the ozone smoothness.
Thereby, the non-linear forward model is approximated with an affine map and the linearised RTE, which in combination with the marginal-then-conditional (MTC) scheme~\cite{fox2016fast} enables efficient inference.
...
For efficient inference we employ the marginal-then-conditional (MTC) scheme as in~\cite{fox2016fast}.
...
%Depending on the problem and the available model,
...
This approach, known as the marginal-then-conditional (MTC) method~\cite{fox2016fast}, is particularly advantageous when $\bm{x}\in \mathbb{R}^n$ is high-dimensional, while $\bm{\theta}$ is low-dimensional and the evaluation of the marginal posterior
\begin{align}
...
In the case of a linear-Gaussian hierarchical Bayesian model, as in \cite{fox2016fast}, both the marginal distribution $\pi (\bm{\theta}| \bm{y})$ 
...
\begin{figure}[htb!]
	...
	\begin{tikzpicture}
		...
		every edge/.style = {draw, -latex, thick} % <---
		\node[roundnode2] at (-2.25,4.75) (Q)     {$\bm{Q}$};
		\node[roundnode2] at (-1,3.5) (x)     {$\bm{x}$};
		\node[roundnode2] at (-1,1.75) (A)    {$\bm{A}( \bm{x})$};
		\node[rectnode] at (-1,0) (y)    {$\bm{y}$};
		%\node[roundnode2] at (-2.5,2.5) (e)    {$\bm{\eta}$};
		\node[roundnode2] at (-4.5,4.75) (S)    {$\bm{\Sigma}$};
		\node[roundnode2] at (-5.5,6) (s)    {$\gamma$};
		\node[roundnode2] at (-3.5,6) (d)    {$\delta$};
...
We draw $m-1 = 29$ posterior samples in $\approx 0.003$.
...
\begin{figure}[htb!]
	...
	\begin{tikzpicture}
		...
		\node[roundnode2] at (-1,3) (A)    {$\bm{A} (\bm{x})$};

		\node[rectnode] at (-1,0.5) (y)    {$\bm{y}$};

		\node[roundnode2] at (-3.25,5) (e)    {$\bm{\Sigma}$};
		\draw[->, very thick] (e) -- (y);
		\draw[->, mydotted,  very thick] (A) -- (y);
		\draw[->, mydotted,  very thick] (x) -- (A);

		
		\node[roundnode2] at (4,5) (x)     {$\bm{x}$};

		\node[roundnode2] at (5,4) (AL)    {$\bm{A}_L\bm{x}$};
		\node[roundnode2] at (5,2) (Aff)    {$\bm{M}\bm{A}_L\bm{x}$};
		\node[rectnode] at (4,0.5) (y)    {$\bm{y}$};
		
		\node[roundnode2] at (1.75,5) (e)    {$\bm{\Sigma}$};
		\draw[->, very thick] (e) -- (y);


		
		\draw[->,  mydotted, very thick] (x) edge[bend left=40] (AL);  
		\draw[->,  mydotted, very thick] (AL) -- (Aff);  
		\draw[->, mydotted,very thick] (Aff) edge[bend left=40] (y);  
