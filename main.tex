...
\textcolor{red}{Some satellites, which orbit around the earth at a height of around 500km above the ground, carry measurement devices to determine trace gas concentrations in the stratosphere.
...
\textcolor{red}{Measurements of such devices can be described by the radiative transfer equation (RTE), which in this case is a path integral along the satellite's pointing direction.
...
\textcolor{red}{Existing methods use regularisation methods to retrieve trace gas concentration from measurements~\cite{rodgers1976retrieval, livesey2006retrieval, raspollini2021level}.
...
\textcolor{red}{Firstly, given some simulated data, we treat this non-linear problem as a linear problem by neglecting the absorption term in the RTE.
...
\textcolor{red}{Using the results of the linearised problem, an affine map approximating the non-linear forward model is obtained.
...
\textcolor{red}{All programming and analysis in this paper are done in Python, and the reported computation times are taken on a MacBook Pro from 2019 with a 2.4 GHz quad-core Intel i5 processor.}
...
\begin{figure}[ht!]
	\centering
	\begin{tikzpicture}
		\node[roundnode2] at (0,2.5) (th)    {$\bm{\theta}$};
		\node[roundnode2] at (0,0.5) (x)    {$\bm{x}$};
		\node[align=center] at (0,-1.5) (u)    {$\{\bm{A}(\bm{x})\}$};
		...
		\node[align=center] at (2.8,2.5) (tht) {$\sim \pi_{\bm{\theta}}(\cdot) $ hyper-parameters};
...
\begin{figure}[htb!]
	...
	\begin{tikzpicture}
		...
		\node[align=center] at (-1,3.25) (A)    {$\bm{A}( \bm{x})$};
