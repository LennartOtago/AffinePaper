...
\usepackage[backend=biber,style=numeric,natbib=true,maxbibnames=5,giveninits=true]{biblatex}
...
This takes roughly $\approx 0.1$s on a MacBook Pro from 2019 with a 2.4 GHz quad-core Intel i5 processor.
\textcolor{red}{Integrated Autocorrelation Time -- MH-correction step??}
...
We draw $n-1 = 33$ samples to find an affine approximation to the non-linear forward model in $\approx 0.1$s on a MacBook Pro from 2019 with a 2.4 GHz quad-core Intel i5 processor.
...
\begin{align}
   \overline{\text{CV}} =  \left\lVert \frac{1}{n} \frac{\sqrt{ \text{var}_N(\bm{\mu}_{\bm{x}|\bm{y}})}}{\bm{\mu}_{\bm{x}|\bm{y}} }\right\rVert_1 ,
\end{align}
...
Further we conclude 20 full conditional posterior ozone samples are enough for a sufficient estimate of the STD.
