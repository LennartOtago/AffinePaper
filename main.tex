...
\section{Results}
Here the Results are represented.
...
An Affine map is obtained and the non-linear forward model is approximated.
Again the marginal posterior is calculated on the same predefined grid.
...
Further, at no additional computational cost the posterior mean in Eq.~\ref{eq:MeanInt} is obtained by weighted expatiation over the predefined grid with $\pi(\lambda| \bm{y}) = \int \pi(\lambda, \gamma| \bm{y}) \diff \gamma $.
...
We sample from the marginal posterior via a scheme similar to the inverse Rosenblatt transport (IRT) as in~\cite{dolgov2020approximation}.
...
\end{align}
Note, that alternatively on could draw an independent sample using Eq.~\ref{eq:GibbsStep}, which is a Gamma distribution due to the choice of prior.
The here presented IRT method works for arbitrary priors.

We define the Integrated Autocorrelation Times (IACT) as in \cite{fox2016fast} so that
\begin{align}
	\tau_{\text{int}}  \coloneqq  \Bigg(  1 + 2 \sum_{t = 1}^{W} \frac{\Gamma(t)}{\Gamma(0)}  \Bigg)  
\end{align} 
with autocorrelation coefficient $\Gamma(t)$.
...
The IATCs are $\tau_{\text{int},\lambda} = 1.05\pm-0.042$ and $\tau_{\text{int},\gamma} =0.985\pm0.034 $ based on a chain with 10000 samples.
So every second sample via the IRT scheme presents an independent marginal posteiror sample.
...
\textcolor{red}{MH-correction step??}
...
\begin{align}
	...
	&\propto \exp \left(-\frac{1}{2} (\bm{A} \bm{x} - \bm{y})^T \bm{\Sigma}^{-1}_{\gamma} (\bm{A}  \bm{x} - \bm{y})\right) \exp \left(-\frac{1}{2} (\bm{\mu} - \bm{x} )^T \bm{Q}_{\delta}(\bm{\mu} - \bm{x} ) \right),\\
...
\begin{align}
	...
	\end{bmatrix} , 
\end{align}
$\bm{Q}_{\delta} = \delta \bm{L}$ is the prior precision, $\bm{\mu} = \bm{0}$ the prior mean and $\bm{\Sigma}_{\gamma} = \gamma^{-1} \bm{I}$ the noise covariance.
...
\begin{align}
	\label{eq:RTO}
	\left( \bm{A}^T \bm{\Sigma}^{-1}_{\gamma} \bm{A} + \bm{Q}_{\delta} \right) \bm{x}^{(k)} = \bm{A}^T \bm{\Sigma}^{-1}_{\gamma} \bm{y} + \bm{Q}_{\delta} \bm{\mu} + \bm{v}_1 + \bm{v}_2,
\end{align}
%where the term $-\hat{\bm{A}}^T \bm{b}$ is decomposed as $\bm{v}_1 + \bm{v}_2$, 
with $\bm{v}_1 \sim \mathcal{N}(\bm{0}, \bm{A}^T \bm{\Sigma}^{-1}_{\gamma} \bm{A})$ and $\bm{v}_2 \sim \mathcal{N}(\bm{0}, \bm{Q}_{\delta})$, representing independent Gaussian random variables~\cite{bardsley2012mcmc, fox2016fast}.
...
We draw $n-1 = 29$ samples to find an affine approximation to the non-linear forward model in $\approx 0.003$s on a MacBook Pro from 2019 with a 2.4 GHz quad-core Intel i5 processor.
...
\begin{figure}[ht!]
	...
	\caption[]{Functions $f(\lambda)$ and $g(\lambda)$ for $\lambda = [10^{-5},8 \times 10^{-4}] $, grid points are marked red.}
	\label{fig:fandg}
\end{figure} 
\begin{figure}[ht!]
	\centering
...
This takes $\approx 0.1$s on a MacBook Pro from 2019 with a 2.4 GHz quad-core Intel i5 processor.
...
The IATCs are $\tau_{\text{int},\lambda} = 0.968 \pm0.034$ and $\tau_{\text{int},\gamma} =0.954\pm0.033$ based on a chain with 10000 samples.

Samples from the conditional posterior are drawn via the RTO method.
To estimate the number of samples $N$ needed for a good enough estimate of the STD we calculate the average Coefficient of Variation ($\overline{\text{CV}}$),
\begin{align}
   \overline{\text{CV}} =  \left\lVert \frac{1}{n} \frac{\sqrt{ \text{var}_N(\bm{\mu}_{\bm{x}|\bm{y}})}}{\bm{\mu}_{\bm{x}|\bm{y}} }\right\rVert_1 , \label{eq:avrgCV}
\end{align}
...
\begin{align}
    \text{var}_N(\mu_{\bm{x}|\bm{y}}) = \sum^N_{k=1}\frac{(\bm{x}^{(k)} - \mu_{\bm{x}|\bm{y}})^T (\bm{x}^{(k)} - \mu_{\bm{x}|\bm{y}})}{N}
\end{align}
is calculated with the posterior mean given by weighted expectations as in Eq.~\ref{eq:MeanInt}.
Alternatively one could evaluate the posterior covariance as weighted expectation over the marginal posterior grid (see Eq.~\ref{eq:CovInt}).

The $\overline{\text{CV}}$ is calculated for each number of samples ranging from $1$ to $10^4$ and plotted in Fig.~\ref{fig:Err}.
\begin{figure}[ht!]
	\centering
	...
	\label{fig:Err}
\end{figure}
In Fig.~\ref{fig:Err} we observe that the $\overline{\text{CV}}$ is rather large indicating that the STD is large compared to the posterior mean.
Further, we conclude 50 full conditional posterior ozone samples are enough for a sufficient estimate of the STD.
Approximating the marginal posterior on a $20 \times 20$ grid and taking 50 independent full posterior samples of ozone takes $\approx 0.008$s on a MacBook Pro from 2019 with a 2.4 GHz quad-core Intel i5 processor.
...
\begin{itemize}
	...
	\item $29$ samples of full conditional are enough  for finding an affine map, this includes $29$ solves of ``randomised'' $x_{\lambda}$ via the RTO method (see Eq.~\ref{eq:RTOAppl})
	\item $50$ samples of full conditional are enough for the STD estimate of posterior ozone, this includes $50$ solves of ``randomised'' $x_{\lambda}$ via the RTO method (see Eq.~\ref{eq:RTOAppl})
\end{itemize}
\clearpage
